...
export function popUpImage(sentence: ISentence): IPerform {
  // 根据参数设置指定位置
  ...
  if (key !== '') {
    ...
    if (pos === 'center') {
      if (webgalStore.getState().stage.popUpImageName === sentence.content) {
        isRemoveEffects = false;
      }
    }
    if (pos === 'left') {
      if (webgalStore.getState().stage.popUpImageNameLeft === sentence.content) {
        isRemoveEffects = false;
      }
    }
    if (pos === 'right') {
      if (webgalStore.getState().stage.popUpImageNameRight === sentence.content) {
        isRemoveEffects = false;
      }
    }
  }
  ...
  if (isFreeFigure) {
    ...
    if (motion) {
      dispatch(stageActions.setPopUpImageLive2dMotion({ target: key, motion }));
    }
    if (expression) {
    ...
    const dispatchMap: Record<string, keyof IStageState> = {
      ...
      left: 'popUpImageNameLeft',
      right: 'popUpImageNameRight',
    };
